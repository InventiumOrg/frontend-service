...
export default function SignUpForm() {
  ...
  // Handle OAuth sign up (Google, X/Twitter)
  const handleOAuthSignUp = async (provider: "oauth_google" | "oauth_twitter") => {
    if (!signUp) return;
          ...
          <div>
            {!verificationStep && (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
                ...
                <button 
                  type="button"
                  onClick={() => handleOAuthSignUp("oauth_twitter")}
                  disabled={isLoading}
                  className="inline-flex items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
              ...
              <form onSubmit={handleEmailSignUp}>
                <div className="space-y-5">
                  ...
                  <div>
                    <button 
                      type="submit"
                      disabled={isLoading || !firstName || !lastName || !email || !password || !isChecked}
                      className="flex items-center justify-center w-full px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isLoading ? (
                        <div className="flex items-center gap-2">
